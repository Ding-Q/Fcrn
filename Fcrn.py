...
class Fcrn(nn.Module):
    def __init__(self, dim=opt.dim):
        ...
        self.UpProject_2                 = UpProject(4*dim, 2*dim)
        self.UpProject_3                 = UpProject(2*dim, 2*dim)
        self.UpProject_4                 = UpProject(2*dim, 1*dim)
    ...
    def forward(self, x):
        ...
        x = self.UpProject_2(x)
        x = self.UpProject_3(x)
        x = self.UpProject_4(x)
...
if opt.load_model == 'True':
    fcrn.load_state_dict(torch.load('./model_fcrn/fcrn.pkl'))
...
def test(device, test_dataloader):
    ...
    for batch_idx, (road, road_label)in enumerate(test_dataloader):
        ...
        det_road = fcrn(road, road.shape[0])
    ...
    if opt.mode == 'train':
        ...
        for epoch in tqdm(range(num_epochs)):
            ...
            if (epoch + 1) % 100 == 0:
                ...
                torch.save(fcrn.state_dict(), './model_fcrn/fcrn.pkl')
                print('testing...')
                test(device, test_dataloader)
                iou('../test1/lab_detefcrn', '../test1/labw1')
